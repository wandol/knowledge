...
(function($){
    ...
    Highlighter.prototype.highlight = function() {
        ...
        if (!$.isEmptyObject(this.settings.words) && !$.isEmptyObject(this.settings.ranges)) {
        	var range = this.settings.ranges[0];
        	
        	var beforeText = text.substring(0, range.start);
        	var targetText = text.substring(range.start, range.end);
        	var afterText = text.substring(range.end, text.length);
            ...
            $.each(this.settings.words, function(color, words) {
              ...
              var wordMatches = targetText.match(regex);
              if (wordMatches) {
                var evaluated = [];
	        ...
	        mark += ">";
	        targetText = mark + targetText + '</mark>';
	        text = beforeText + targetText + afterText;
        } else if (!$.isEmptyObject(this.settings.words)) {
        	var matches = [];
            $.each(this.settings.words, function(color, words) {
              var wordsRe = htmlDecode(words.join("|"));
              var re = that.spacer + '(' + wordsRe + ')' + that.spacer;
                    ...
                    text = text.replace(
                      new RegExp(match, that.regParam), 
                        function(innerMatch, start, contents) {
                          ...
                          //return '<mark style="font-weight: bold;">' + encodedMatch + '</mark>';
                          return '<mark style="background-color:'+ color +';">' + encodedMatch + '</mark>';
                        }
                      );
    ...
    Highlighter.prototype.setWordsByRanges = function(words, ranges) {
        this.setOptions({ words: words, ranges: ranges });
    };
    ...
    Highlighter.prototype.updateCss = function() {
        ...
        Utilities.cloneCss(this.$el, this.$highlighter, [
            ...
            'vertical-align','word-spacing','text-align','letter-spacing', 'text-rendering','line-height'
        ]);
