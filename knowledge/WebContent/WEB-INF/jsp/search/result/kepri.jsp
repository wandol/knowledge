	...
	<article class="col-xs-12">
		<div class="jarviswidget" id="wid-id-0" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-fullscreenbutton="false" data-widget-custombutton="false" data-widget-sortable="false">
			<header class="ui-sortable-handle">
				<h2>탐색결과</h2><div class="search-rezult">/  ${kepriTotal }건</div>
			</header>
				...
				<div class="row">
					...
					<div id="gallery1_t" class="page">
						<div class="listDt2 listDt2-padd">
							<ul>
								<c:forEach var="result" items="${kepriList}" varStatus="status">
									<li>
									<dl class="listDt">
										<dt onclick="location.href='../problemFocus/pfSearch.do?category=KEPRI&kwd=&pageNum=1&sort=r&powerComp=${result.POWER_COMP_NM}&powerSt=${result.POWER_ST_NM}&stNo=${result.ST_NO}'" style="cursor:pointer">
											<!-- 이미지 영역 -->
 											<%-- <c:set var="imgList" value="${fn:replace(result.IMG_FILES, '|', '`')}"/>
 											<c:set var="imgList" value="${fn:replace(imgList, imgPath, '')}"/>
											<c:set var="img" value="${fn:split(imgList,'`')[0]}" />
											<c:set var="imgLength" value="${fn:length(fn:split(imgList,'`'))}" />
											<c:if test="${not empty img }">
												<img src="../comm/displayImg.do?fileName=${img }" data-img="<c:url value="/images/superbox/superbox-full-1.jpg" />" class="superbox-img" onclick="javascript:showImageList('${imgList}')">
											...
											<ul class="srhrzt">
												<li>
												<strong class="font-lg title_link" onclick="location.href='../problemFocus/pfSearch.do?category=KEPRI&kwd=&pageNum=1&sort=r&powerComp=${result.POWER_COMP_NM}&powerSt=${result.POWER_ST_NM}&stNo=${result.ST_NO}'" style="cursor:pointer">${result.REPORT_NM }</strong> 
