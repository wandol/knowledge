...
/**
 * Class Name : GenPrevDAO.java
 * Description : 검색대상 Gen Prev 조회
 *
 ...
 */
@Repository
public class GenPrevDAO {
	private static final Logger logger = LoggerFactory.getLogger(GenPrevDAO.class);
	...
	public RestResultVO getDetailData(ParameterVO paramVO)  throws Exception {
		...
		sbLog.append(  dcUtil.getLogInfo("ranking",
				"발전기-예방진단" ,
				commonUtil.null2Str( paramVO.getUserId(),""), paramVO.getRepoName() + "#@@#" + paramVO.getKwd() + "#@@#" + "GEN_PREV",
				paramVO.getPageNum(),
				false,
				paramVO.getSort(),
				commonUtil.null2Str( paramVO.getRecKwd(),"" )) );
