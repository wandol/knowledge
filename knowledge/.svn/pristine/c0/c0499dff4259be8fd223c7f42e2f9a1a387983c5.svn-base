...
/**
 * Class Name : StTurbine.java
 * Description : 검색대상 ST TURBINE 조회
 *
 ...
 */
@Repository
public class StTurbineDAO {
	private static final Logger logger = LoggerFactory.getLogger(StTurbineDAO.class);
	...
	public RestResultVO getDetailData(ParameterVO paramVO)  throws Exception {
		...
		sbLog.append(  dcUtil.getLogInfo("ranking",
				"증기터빈" ,
				commonUtil.null2Str( paramVO.getUserId(),""), paramVO.getRepoName() + "#@@#" + paramVO.getKwd() + "#@@#" + "ST_TURBINE",
				paramVO.getPageNum(),
				false,
				paramVO.getSort(),
				commonUtil.null2Str( paramVO.getRecKwd(),"" )) );
