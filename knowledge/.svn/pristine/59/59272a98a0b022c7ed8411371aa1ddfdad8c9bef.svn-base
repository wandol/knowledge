		...
		<div class="jarviswidget" id="wid-id-0" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-fullscreenbutton="false" data-widget-custombutton="false" data-widget-sortable="false">
			<header class="ui-sortable-handle">
				...
				<div id="gallerytab" class="btnr" role="menu">
					<a href="<c:url value="/problemFocus/pfSearch.do?category=ST_TURBINE"/> " id="gallery1" class="btn <c:if test="${params.isImg ne 'Y' }">on</c:if>"><i class="fa fa-list"></i>목록보기</a>
					<a href="<c:url value="/problemFocus/pfSearch.do?category=ST_TURBINE&isImg=Y"/> " id="gallery2" class="btn <c:if test="${params.isImg eq 'Y' }">on</c:if>"><i class="fa  fa-th-large"></i>이미지보기</a>
				</div>
			</header>
										...
										<dt>
											...
											<a href="javascript:detailPFReport('${result.MD5_KEY }', '${result.GUBUN_NO }')">
												<ul>
													...
													<li>진단부위 : ${fn:replace(result.PARTNAME_KWD, '|', '  ,') }</li>
													<li>위치 : ${fn:replace(result.PART2NAME_KWD2, '|', '  ,') }</li>
													<li>현상 : ${fn:replace(result.SYMPTOM_KWD, '|', '  ,') }</li>
<%-- 													<li>대책 : ${fn:replace(result.ACTION_KWD, '|', '  ,') }</li> --%>
												</ul>
											</a>
											</dd>
									</dl>
