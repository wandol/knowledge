...
/**
 * Class Name :PerformDAO.java
 * Description : 검색대상 perform 조회
 *
...
public class PerformDAO {
	private static final Logger logger = LoggerFactory.getLogger(PerformDAO.class);
	...
	public RestResultVO getDetailData(ParameterVO paramVO)  throws Exception {
		...
		sbLog.append(  dcUtil.getLogInfo("ranking",
				"성능" ,
				commonUtil.null2Str( paramVO.getUserId(),""), paramVO.getRepoName() + "#@@#" + paramVO.getKwd() + "#@@#" + "PERFORM",
				paramVO.getPageNum(),
				false,
				paramVO.getSort(),
				commonUtil.null2Str( paramVO.getRecKwd(),"" )) );
	...
	/*
	 * 성능 검색결과에서 컬럼정보를 가져온다.
	 */
	public List<String> selectTestPeriodGroupBy(Map<String, Object> map) throws Exception {
		return sqlSession.selectList("KepriMapper.selectTestPeriodGroupBy", map);
	}
	
	/*
	 * 성능 검색결과에서 테스트항목 정보를 가져온다.
	 */
	public List<String> selectTestItemGroupBy(Map<String, Object> map) throws Exception {
		return sqlSession.selectList("KepriMapper.selectTestItemGroupBy", map);
	}
	
	/*
	 * 성능 검색결과를 가져온다.
	 */
	public List<Map<String,String>> selectPerform(Map<String, Object> map) {
		return sqlSession.selectList("KepriMapper.selectPerform", map);
	}
	
	/*
	 * 성능 검색결과 총 카운트를 가져온다.
	 */
	public int selectPerformCount(Map<String, Object> map) {
		return sqlSession.selectOne("KepriMapper.selectPerformCount", map);
	}
}
